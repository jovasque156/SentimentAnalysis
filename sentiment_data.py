...
class SentimentExampleBatchIterator:
    ...
    def get_next_batch(self):
        if self._cur_idx < len(self.data): # loop over the dataset
            ...
            #Go along batch_exs
            for ex in batch_exs:
                words_indices, label = ex.word_indices, ex.label
                length = len(words_indices)
                
                batch_lengths += [length]
                batch_labels += [label]

            #Padding
            max_len = max(batch_lengths)
            for ex in batch_exs:
                words_indices = ex.word_indices

                batch_inputs.append(words_indices + list(np.repeat(self.PAD_idx, max_len-len(words_indices))))
                
            #Cast to numpy
            batch_lengths = np.array(batch_lengths, dtype='int64')
            batch_labels = np.array(batch_labels, dtype='int64')
            batch_inputs = np.array(batch_inputs, dtype='int64')
